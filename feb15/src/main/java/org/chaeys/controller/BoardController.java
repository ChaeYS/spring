...
public class BoardController {
	...
	public String board(@RequestParam(value="pageNo", required=false) String no, Model model) {
		...
		paginationInfo.setPageSize(10);//페이징 리스트의 사이즈 (하단에 10페이지까지 보이고 다음 버튼 누르면 11페이지부터 보임)
	...
	//2024-02-15 psd 2024-02-16 웹표준 기술 psd
	@GetMapping("/detail")
	public String detail(@RequestParam(value = "no", defaultValue = "10") String no, Model model) {
		//String no = request.getParameter("no");
		//System.out.println(util.str2Int(no)); //숫자면 자신의 숫자, 문자면 0이 출력
	...
	public String write() {
		//return "write";
		return "redirect:/login?error=2048"; //한글로 "잘못된 접근입니다"는 쓸 수 없다.
	}
	
	//글쓰기 2024-02-16
	@PostMapping("/write") //내용 + 제목 받아서 -> DB에 저장 -> 보드로
	...
	@GetMapping("/likeUp")
	public String likeUp(@RequestParam("no") String no, @RequestParam("cno") String cno) { //board_no=no, cno=cno로 날아온다.
