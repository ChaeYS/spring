...
public class Util {
	...
	// 2024-02-21 psd 웹표준
	public HttpServletRequest req() { // return타입이 HttpServletRequest
		ServletRequestAttributes sra = (ServletRequestAttributes) RequestContextHolder.currentRequestAttributes();
		HttpServletRequest request = sra.getRequest(); // 현재 접속한 사람의 request를 가져올 수 있다.
		return request;
	}
	...
	public void sendEmail(String email, String key) throws EmailException { 
		...
		simpleEmail.setStartTLSEnabled(true); //인증 방법 = 고정
		simpleEmail.setFrom("", "트리니티 코인"); //보내는 사람 email, 보내는 사람 이름 = 고정
		simpleEmail.addTo(email); //받는 사람 
		simpleEmail.setSubject(""); //제목
		simpleEmail.setMsg("인증번호는 [" + key + "] 입니다"); //본문 내용 text
		simpleEmail.send(); //전송 
	}
